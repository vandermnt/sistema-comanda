...
<div class="col-sm-12">
  <form method="post" action="{{ action('PedidoController@salvarPedidoLevar') }}">
    ...
    <input type ="hidden" name="categoria" value="chopp">

    <select id="op" name="op" class="form-control form-control-lg" onchange="optionCheck()" style="width: 100%">
      <option disabled="true" selected="true"> Escolha o chopp </option>
      @foreach($chopp as $chopp)
        <option value="{{ $chopp->id }}"> {{ $chopp->nome }} </option>
      @endforeach
    </select>
