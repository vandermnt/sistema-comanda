...
<div class="col-sm-12">
  <form method="post" action="{{ action('PedidoController@salvarPedidoLevar') }}">
    ...
    <input type ="hidden" name="categoria" value="porclassica">

    <select id="op" name="op" class="form-control form-control-lg" onchange="optionCheck()" style="width: 100%">
      <option disabled="true" selected="true"> Escolha a porção clássica </option>
      @foreach($porclassica as $porclassica)
        <option value="{{ $porclassica->id }}"> {{ $porclassica->nome }} </option>
      @endforeach
    </select>
